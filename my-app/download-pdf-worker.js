...
async function copyWorkerFile() {
  ...
  try {
    const workerFiles = await glob("**/pdf.worker*.js", {
      cwd: nodeModulesPath,
    });

    if (workerFiles.length > 0) {
      // Prefer files with 'legacy' in the path
      const legacyWorkerFile = workerFiles.find((file) =>
...
`;
    ...
    console.warn(
      "Warning: This is a stub file. You may need to manually configure PDF.js worker in your application."
    );
    return false;
  } catch (error) {
...
copyWorkerFile().then((success) => {
  if (!success) {
    console.log(
      "Worker file handling completed, but with fallback to stub file."
    );
  }
});
